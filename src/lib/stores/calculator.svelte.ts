...
import { redistributeLower, computeAutoTop, snapToStep, clamp } from '$lib/logic/redistribute';
...
class CalculatorStore {
	...
	topPayment = $derived(this.topOverridden ? this.topGrade?.payment ?? 0 : this.autoTop);
	actualTotal = $derived(
	...
	addGrade() {
		const lastNum = this.grades.length > 0 ? this.grades[this.grades.length - 1].num : 0;
		const lastPayment = this.grades.length > 0 ? this.grades[this.grades.length - 1].payment : 0;
